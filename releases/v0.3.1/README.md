...
**▶ Live demo (stable, v0.3.1): https://georgekorpas.github.io/tone-lab/**
...
Tone Lab models three distinct sound-engine families, switched at the top-right of the header:
...
- **D3** — a single macro-oscillator engine, **PL**, inspired by Mutable Instruments Plaits (Émilie Gillet, MIT). Models are added incrementally; the first build ships *VA* (virtual analog). See `LICENSES.md` for full attribution.
...
The version pill `v0.3.1` next to the title shows the build, and the discrete tip strip in the header centre cycles through 12 sound-design tips when you click ‹ ›.
...
The pill `v0.3.1` next to the title shows the major version. The browser-readable build tag (in DevTools → Elements → `<head>`) under `<meta name="tonelab-build">` shows the precise build identifier — useful for verifying you've reloaded after an update.
